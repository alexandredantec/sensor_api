...
class TestVisitSensor(unittest.TestCase):
    ...
    def test_with_break(self):
        ...
        visit_count = visit_sensor.get_visit_count(date(year=2023, month=9, day=22))

        self.assertEqual(visit_count, second=0)

    def test_with_malfunction(self):
        visit_sensor = VisitSensor(avg_visit=1200, std_visit=300, perc_malfunction=10)
        visit_count = visit_sensor.get_visit_count(date(year=2023, month=9, day=28))

        self.assertEqual(visit_count, second=248)
